...
### 1. ALWAYS Fetch Remote File Before Editing

**Before modifying ANY Home Assistant configuration file, ALWAYS pull the current version from the remote instance first.** The remote instance is the source of truth - local files may be stale.
...
**Never skip this step.** The remote may have been edited via the HA UI, another user, or a previous session.

### 2. Verify Sensors and Entities Exist

**Before building any dashboard card or integration config, verify that referenced entities actually exist on the instance.**
...
hass-cli state list | grep "^binary_sensor\."
hass-cli state list | grep "^light\."
```

**If a sensor doesn't exist:** Stop and inform the user. Suggest how to create it or which integration provides it.

### 3. Prefer HACS Components

**When building dashboards or integrations, always check if a HACS (Home Assistant Community Store) component exists for the use case before using built-in alternatives or manual implementations.**

- Check HACS for custom cards (e.g., mushroom, mini-graph, auto-entities, stack-in-card)
- Check HACS for custom integrations before writing manual sensors/automations
- If a HACS component is available and mature, prefer it over manual implementation
- Note any HACS dependencies in the deployment so the user can install them

### 4. Maintain Consistent Dashboard Look and Feel

**Unless the user explicitly requests a visual change, match the existing dashboard style:**

- Use the same card types already present in the dashboard (e.g., if mushroom cards are used, keep using mushroom)
- Match existing color schemes, icon styles, and layout patterns
- Preserve grid column counts, spacing, and section organization
- Keep the same card configuration patterns (fill_container, collapsible_controls, etc.)
- When adding new sections, follow the naming and ordering conventions already established

**Before adding cards to an existing dashboard:**
1. Fetch the current dashboard file (Rule 1)
2. Analyze the existing card types, styles, and patterns
3. Match new cards to the established style
4. Only deviate if the user explicitly asks for a different look
...
IMPORTANT: If you update a dashboard, use browser mod to refresh the browsers so they get the latest version. If you need to restart to make changes take effect run the browser mod refresh AFTER the reboot. If you dont need to reboot running the browser mod refresh will do.
...
**Default Dashboard Location: UI Lovelace Minimalist**

When the user asks to edit "a dashboard" or "the dashboard", they are most likely referring to the **UI Lovelace Minimalist (YAML mode)** dashboards located on the remote instance at:

**Card usage docs**: https://ui-lovelace-minimalist.github.io/UI/usage/cards/card_battery/
(Navigate sidebar for all card types, chips, popups, and custom cards)

```
/config/ui_lovelace_minimalist/dashboard/views/
```

These are YAML-based view files (not JSON `.storage/` files). Always fetch these from the remote first:
...
- **Minimalist YAML dashboards**: YAML files in `/config/ui_lovelace_minimalist/dashboard/views/` (primary, most likely what user means)
- **Storage-based dashboards**: JSON files in `.storage/` directory (e.g., `.storage/lovelace.control_center`)
